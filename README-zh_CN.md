...
### 概览
domain-stress 是一款基于域名的压测工具，你可以使用它对某个域名对应的服务进行压测。指定域名和请求总数即可使用。  

当某些时候你缺少后端IP池的负载均衡器 或是你只知道域名时可以使用该工具。  

目前支持HTTP的GET、POST、PUT、DELETE等请求，你可以使用 -n 参数指定请求总数，使用 -c 参数指定每个后端IP的并发数。

发压过程中每秒会输出一次当前已完成的请求情况，测试结束后会输出完整结果。
...
<br> 
test命令 测试延迟，基于ICMP协议 由于某些火墙或主机可能会禁ping 故仅做参考
...
### 待开发内容
× 接口调用方式目前仅支持http，后续需要支持grpc、websocket协议   
× 执行计划生成目前后端IP是平均分配请求，需要支持根据后端IP时延进行分配。

### 协议
本项目采用 [Apache 2.0协议](https://www.apache.org/licenses/LICENSE-2.0) 进行许可。